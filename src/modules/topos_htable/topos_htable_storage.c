		...
		tps_data_t *md, tps_data_t *sd)
{
	...
	ret = snprintf(ptr, TPS_HTABLE_SIZE_KEY, "%.*s,%.*s,%.*s,x%.*s",
			md->s_method.len, md->s_method.s, md->a_callid.len, md->a_callid.s,
			md->b_tag.len, md->b_tag.s, xuuid.len, xuuid.s);
...
static int tps_htable_load_initial_method_branch(tps_data_t *md, tps_data_t *sd)
{
	...
	ptr = _tps_htable_key_buf;
	ret = snprintf(ptr, TPS_HTABLE_SIZE_KEY, "%.*s,%.*s,%.*s,x%.*s",
			smethod.len, smethod.s, md->a_callid.len, md->a_callid.s, xtag.len,
			xtag.s, xuuid.len, xuuid.s);
	if(ret < 0 || ret >= TPS_HTABLE_SIZE_KEY) {
		LM_ERR("failed to build htable key\n");
		return -1;
	}
	...
	if(hval != NULL) {
		...
		LM_DBG("AFTER LOAD %.*s", sd->x_vbranch1.len, sd->x_vbranch1.s);
		pkg_free(hval);
	} else {
		// not found
		return 1;
...
int tps_htable_insert_branch(tps_data_t *td)
{
	...
	ret = snprintf(ptr, TPS_HTABLE_SIZE_VAL,
			...
			td->x_via.len, td->x_via.s, td->x_vbranch1.len, td->x_vbranch1.s,
			td->x_rr.len, td->x_rr.s, td->y_rr.len, td->y_rr.s, td->s_rr.len,
			td->s_rr.s, td->x_uri.len, td->x_uri.s, td->x_tag.len, td->x_tag.s,
			td->s_method.len, td->s_method.s, td->s_cseq.len, td->s_cseq.s,
			td->a_contact.len, td->a_contact.s, td->b_contact.len,
			td->b_contact.s, td->as_contact.len, td->as_contact.s,
			td->bs_contact.len, td->bs_contact.s, td->a_tag.len, td->a_tag.s,
			td->b_tag.len, td->b_tag.s, td->x_context.len, td->x_context.s);
	if(ret < 0 || ret >= TPS_HTABLE_SIZE_VAL) {
		...
		sip_msg_t *msg, tps_data_t *md, tps_data_t *sd, uint32_t mode)
{
	...
	if(sd->a_uuid.len <= 0 && sd->b_uuid.len <= 0) {
		LM_ERR("no uuid for this message\n");
		return -1;
	}
	...
	if(ret < 0 || ret >= TPS_HTABLE_SIZE_KEY) {
		LM_ERR("failed to update branch\n");
		return -1;
	}
...
int tps_htable_insert_dialog(tps_data_t *td)
{
	...
	ret = snprintf(ptr, TPS_HTABLE_SIZE_VAL,
			...
			td->s_rr.len, td->s_rr.s, td->iflags, td->a_uri.len, td->a_uri.s,
			td->b_uri.len, td->b_uri.s, td->r_uri.len, td->r_uri.s,
			td->a_srcaddr.len, td->a_srcaddr.s, td->b_srcaddr.len,
			td->b_srcaddr.s, td->s_method.len, td->s_method.s, td->s_cseq.len,
			td->s_cseq.s, td->x_context.len, td->x_context.s);
	if(ret < 0) {
		LM_ERR("failed to build htable val\n");
		return -1;
	}
		...
		sip_msg_t *msg, tps_data_t *md, tps_data_t *sd, uint32_t mode)
{
	...
	// insert hval
	ret = tps_htable_insert_dialog(&hval);
...
int tps_htable_end_dialog(sip_msg_t *msg, tps_data_t *md, tps_data_t *sd)
{
	...
	if((md->s_method_id != METHOD_BYE)
		...
		LM_INFO("no method for ending dialog\n");
		return 0;
	}
