		...
		struct sip_msg *msg, peer_reponse_t *resp, dmq_node_t *node)
{
	...
	*pres_dmq_recv = 1;
	...
	if(jdoc.root->child->type == srjson_Array && strcmp(jdoc.root->child->string, "multi") == 0) {
...
int pres_dmq_build_json_presentity(srjson_doc_t jdoc, srjson_t *jobj, presentity_t *presentity, str *body,
		int t_new, str *cur_etag, char *sphere, str *ruid)
{
	srjson_t *p_json;
	...
	srjson_AddNumberToObject(
			&jdoc, jobj, "action", PRES_DMQ_UPDATE_PRESENTITY);
		...
		int t_new, str *cur_etag, char *sphere, str *ruid, dmq_node_t *node)
{
	...
	pres_dmq_build_json_presentity(jdoc, jdoc.root, presentity, body,
		t_new, cur_etag, sphere, ruid);
...
int pres_dmq_build_json_subscription(srjson_doc_t jdoc, srjson_t *jobj, subs_t *subscription)
{
	srjson_t *s_json;
	...
	if(subscription->expires != 0
		...
		srjson_AddNumberToObject(&jdoc, s_json, "expires",
				subscription->expires + ksr_time_sint(NULL, NULL));
		res += snprintf(NULL, 0, "%u", subscription->expires + ksr_time_sint(NULL, NULL));
	} else {
		srjson_AddNumberToObject(
		...
		while(ptn != NULL) {
			...
			if(presentity != NULL) {
				...
				if(pres_dmq_batch_msg_pres > 1) {
					...
					pres_dmq_build_json_presentity(jdoc, p_json, presentity,
					...
					if(p >= pres_dmq_batch_msg_pres || (pres_dmq_batch_msg_size > 0 && buf_size > pres_dmq_batch_msg_size)) {
						srjson_AddItemToObject(&jdoc, jdoc.root, "multi", p_arr);
	...
	for(i = 0; i < shtable_size; i++) {
		...
		while(s) {
			...
			if(pres_dmq_batch_msg_subs > 1) {
				...
				pres_dmq_build_json_subscription(jdoc, s_json, s);

				srjson_AddItemToArray(&jdoc, s_arr, s_json);
				sn++;

				if(sn >= pres_dmq_batch_msg_subs || (pres_dmq_batch_msg_size > 0 && buf_size > pres_dmq_batch_msg_size)) {
